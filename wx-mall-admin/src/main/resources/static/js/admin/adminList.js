...
layui.use(['table', 'layer', 'form', 'upload', 'element'], function(){
	...
	table.render({
		...
		cols : [[
			...
			{field : 'password',title : '密码', width:450}, 
			{field : 'headPortrait',title : '管理员头像',templet:'#pic'}, 
...
//			 console.log(d.data.list);
	...
	table.on('tool(adminList)', function(obj) {
		...
		// 修改 开始
			...
			sessionStorage.setItem('adminEdit', JSON.stringify(obj.data));
			...
			layer.open({
				...
				// 4 tips层
				type : 2,
				title : [ '修改管理员信息', 'font-size:20px; color: #009688;' ],
				// 宽高
				area : [ '750px', '350px' ],
				// 内容，每个层都不一样，看layui文档
				content : "/html/admin/adminEdit.html",
				// 图标
				icon : 1,
				// 点×关闭
	...
	form.on('submit(add)', function(data) {
		...
		layer.open({
			...
			type : 2,
			title : [ '添加管理员', 'font-size:24px; color: #488CB6;' ],
			// 宽高
			area : [ '950px', '400px' ],
			// 内容，每个层都不一样，看layui文档
			content : "/html/admin/adminAdd.html",
			// 图标
			icon : 1,
			// 点×关闭
			// 层销毁后触发的回调，无论是确认还是取消，只要层被销毁了，end都会执行，不携带任何参数
